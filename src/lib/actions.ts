...
export async function uploadImage(imageData: string): Promise<{ success: boolean, url?: string, message: string }> {
  try {
    const result = await cloudinary.uploader.upload(imageData, {
      folder: 'bidhi-news',
    });
    return { success: true, url: result.secure_url, message: 'Image uploaded successfully' };
  } catch (error) {
    console.error('Cloudinary Upload Error:', error);
    return { success: false, message: 'Failed to upload image to Cloudinary.' };
  }
}
...
export async function saveReview(rawData: z.infer<typeof FormSchema>, authorId: string, authorEmail: string): Promise<State> {
  ...
  try {
    ...
    if (id) {
      ...
      const postNo = await getNextPostNumber();
      const now = new Date();
      const dateStr = now.toISOString().split('T')[0]; // YYYY-MM-DD

      reviewData.postNo = postNo;
      reviewData.postSlug = `${postNo}-${dateStr}`;
